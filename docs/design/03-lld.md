...
### 2.3 Clients (six) and adversarial set (four)
...
must fail: `drops_last_row` (H3), `wrong_amount_format` (H2), `slow_network_import` (H5;
imports `socket`/sleeps 6 s), `dag_missing_dependency` (H6).
...
itertools, functools, math, string`) and any `open(..., "w")`. A violation is an H1 error
